...
public class PresetDetailsScreen extends Screen {
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
		...
		graphics.centeredText(font, Component.literal(preset.name), width / 2, 14, WHITE);
		graphics.centeredText(font, subtitle(), width / 2, 28, GREY);

		graphics.text(font, Component.translatable("quickrebind.details.column_bind").getString(),
				left, 40, DIM);
		graphics.text(font, Component.translatable("quickrebind.details.column_key").getString(),
		...
		if (status != null) {
			graphics.centeredText(font, status, width / 2, height - 66, statusColor);
		}
	}
	...
	private void drawRow(GuiGraphicsExtractor graphics, BindDiff row, int left, int y) {
		graphics.text(font, fit(KeyNames.bindLabel(row.id), KEY_COLUMN - 6), left, y, labelColor(row));
		graphics.text(font, fit(keyText(row), LIST_WIDTH - KEY_COLUMN - SCROLL_BAR_WIDTH - 6),
	...
	private void drawScrollBar(GuiGraphicsExtractor graphics, int left) {

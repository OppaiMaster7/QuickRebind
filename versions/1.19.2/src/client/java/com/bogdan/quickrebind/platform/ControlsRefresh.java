 ...
 *
 * <p>Except on this version, where there is nothing to do: 1.19.2 rebuilds each
 * row's label from the KeyMapping on every frame, so the screen already follows
 * a preset the moment it is applied. From 1.20.1 on the label is cached and has
 * to be refreshed explicitly.
 *
 * <p>The class still exists here so every version has the same shape and a
 * future port starts from something rather than rediscovering the problem.
 */
public final class ControlsRefresh {
	...
	public static boolean refresh(Screen screen) {
		...
		// Nothing to refresh on 1.19.2 — the labels are rebuilt every frame.
		// Finding the list is still worth reporting, so the self-test can tell
		// "no refresh needed" apart from "the hook stopped working".
